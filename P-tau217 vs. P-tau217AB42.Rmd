...
---
This document contains the code used for analyzing p-tau217/AB42 performances, compared to p-tau217 (Lumipulse).
...
#LUMI p-tau217 VS. p-tau217/AB42 (pooled, analyses Figure 4)

##Boxplots (extended data 4)
```{r boxplots, echo=F}
...
run_analysis <- function(lumi, name) {
  ...
  #Calculate the intermediate participants
 ...
 table(lumi$greyzone_ptau217ab42)
  ...
  f_comp_stats_2cp <- function(data, indices) {
    ...
    d_2  <- d %>% filter(greyzone_ptau217ab42 != 2)
  
    #Create confusion matrix1
    d2_tp=sum(d_2$csf_status==1 & d_2$ptau217_ab42_spec90==1)
    d2_fp=sum(d_2$csf_status==0 & d_2$ptau217_ab42_spec90==1)
    d2_tn=sum(d_2$csf_status==0 & d_2$ptau217_ab42_spec90==0)
    d2_fn=sum(d_2$csf_status==1 & d_2$ptau217_ab42_spec90==0)
    
    #Accuracy1
    acc_ptau217ab42=(d2_tp+d2_tn)/(d2_tp+d2_tn+d2_fn+d2_fp)
    
    #Calculate PPV and NPV (cutpointr)1
    PPV_ptau217ab42=ppv(tp = d2_tp,fp = d2_fp,
            tn = d2_tn,fn = d2_fn)
    
    NPV_ptau217ab42=npv(tp = d2_tp,fp = d2_fp,
            tn = d2_tn,fn =d2_fn)
...
#Comorbidity plots p-tau217 AB42 (extended data 5)
