...
---
This document contains code used for all the stratified analyses for different covariates and comorbidities. 
...
##Calculate Accuracy, PPV, NPV and 95% CIs 1 cutoff ----
...
#Store p-values 1cp in dataframe
...
##Repeat bootstrap in filtered sample, remove intermediate participants, 2 cutoff approach ----
set.seed(12345)
...
merged_gender <- rbind(results_1cp, results_twocp) %>% 
  mutate(Cohort = "Sex") #merge 2 dataframes
...
openxlsx::write.xlsx(merged_gender,file="Fig2_Sex.xlsx", rowNames =T)
...
openxlsx::write.xlsx(pvalues_gender,file="~Fig2_Sex_Pvalues.xlsx", rowNames =T)
...
##Sex
merged_sex <- read_xlsx("Fig2_Sex.xlsx")
merged_sex <- merged_sex[,2:52]
group1_sex <- cbind(merged_sex[0:2, 1:12], merged_sex[0:2, 50:51]) #males
colnames(group1_sex) <- c("AUC", "AUC_ci_low", "AUC_ci_high", "Accuracy", "Accuracy_ci_low", "Accuracy_ci_high", "PPV", "PPV_ci_low","PPV_ci_high", "NPV", "NPV_ci_low", "NPV_ci_high", "G1", "G2")
group1_sex$group <- "1"
group2_sex <- cbind(merged_sex[0:2, 13:24], merged_sex[0:2, 50:51]) #females
colnames(group2_sex) <- c("AUC", "AUC_ci_low", "AUC_ci_high", "Accuracy", "Accuracy_ci_low", "Accuracy_ci_high", "PPV", "PPV_ci_low","PPV_ci_high", "NPV", "NPV_ci_low", "NPV_ci_high", "G1", "G2")
group2_sex$group <- "2"
merged_sex2 <- bind_rows(group1_sex, group2_sex)
merged_sex2$Cohort <- "Sex"
merged_sex2$Cutpoint <- c("1 Cutpoint", "2 Cutpoint","1 Cutpoint", "2 Cutpoint")
...
plotdata <- bind_rows(merged_sex2, merged_education2, merged_CKD2,merged_diabetes2, merged_age2,merged_apoe2) 
...
plotdata <- read.xlsx("Fig4_Plotdata_sensitivity.xlsx")
...
igroup1_sex <- merged_sex[0:2, 34:39]
colnames(igroup1_sex) <- c("Intermediaten", "Intermediaten_ci_low", "Intermediaten_ci_high", "Intermediatep", "Intermediatep_ci_low", "Intermediatep_ci_high")
igroup1_sex$group <- "1"
igroup2_sex <- merged_sex[0:2, 40:45]
colnames(igroup2_sex) <- c("Intermediaten", "Intermediaten_ci_low", "Intermediaten_ci_high", "Intermediatep", "Intermediatep_ci_low", "Intermediatep_ci_high")
igroup2_sex$group <- "2"
imerged_sex <- bind_rows(igroup1_sex, igroup2_sex)
imerged_sex$Cohort <- "Sex"
imerged_sex$Cutpoint <- c("1 Cutpoint", "2 Cutpoint","1 Cutpoint", "2 Cutpoint")
...
plotdata_i <- bind_rows(imerged_sex, imerged_education, imerged_CKD,imerged_diabetes,imerged_apoe, imerged_age)
plotdata_i <- plotdata_i[grep("2 Cutpoint", plotdata_i$Cutpoint), ]

openxlsx::write.xlsx(plotdata_i,file="Fig2_plotdata_intermediate.xlsx", rowNames =T)
...
#Age only, in primary care (Age only, supplementary table 2)
...
#Age only, in Secondary care (Age only, supplementary table 2)
...
```{r split by age pooled secondary care, echo=F}
...
openxlsx::write.xlsx(merged_age,file="Age_pooled_secondary.xlsx", rowNames =T)
...
#Comorbidity comparisons in primary care (all, extended figure 3)
...
merged_sex <- read_xlsx("Sex_BFPC.xlsx")
rownames(merged_sex) <- merged_sex[[1]]  
merged_sex <- merged_sex[ , -1]   

group1_sex <- cbind(merged_sex[0:2, 1:12], merged_sex[0:2, 50:51])
colnames(group1_sex) <- c("AUC", "AUC_ci_low", "AUC_ci_high", "Accuracy", "Accuracy_ci_low", "Accuracy_ci_high", "PPV", "PPV_ci_low","PPV_ci_high", "NPV", "NPV_ci_low", "NPV_ci_high", "G1", "G2")
group1_sex$group <- "1"
group2_sex <- cbind(merged_sex[0:2, 13:24], merged_sex[0:2, 50:51])
colnames(group2_sex) <- c("AUC", "AUC_ci_low", "AUC_ci_high", "Accuracy", "Accuracy_ci_low", "Accuracy_ci_high", "PPV", "PPV_ci_low","PPV_ci_high", "NPV", "NPV_ci_low", "NPV_ci_high", "G1", "G2")
group2_sex$group <- "2"
merged_sex2 <- bind_rows(group1_sex, group2_sex)
merged_sex2$Cohort <- "Sex"
...
#Age
merged_age <- read_xlsx("Age_BFPC.xlsx")
...
###Intermediate
```{r Intermediate figure primary care only, echo=F}
igroup1_sex <- merged_sex[0:2, 34:39]
colnames(igroup1_sex) <- c("Intermediaten", "Intermediaten_ci_low", "Intermediaten_ci_high", "Intermediatep", "Intermediatep_ci_low", "Intermediatep_ci_high")
igroup1_sex$group <- "1"
igroup2_sex <- merged_sex[0:2, 40:45]
